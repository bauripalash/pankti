...
static PValue gfx_DrawRectangle(PVm *vm, PValue *args, u64 argc) {
    ...
    DrawRectangleRec((Rectangle){(int)xVal, (int)yVal, (int)wVal, (int)hVal}, GFX_COLOR_BLACK_CODE);
    return MakeNil();
}
...
static PValue gfx_DrawCircle(PVm * vm, PValue * args, u64 argc){
    ...
    DrawCircle((int)xVal, (int)yVal, (float)rVal, (Color){0,0,0,255/2});
    return MakeNil();
}
...
void PushStdlibGraphics(PVm *vm, SymbolTable *table) {
    StdlibEntry entries[] = {
        ...
        MakeStdlibEntry(GFX_STD_CIRCLE, gfx_DrawCircle, 3),
