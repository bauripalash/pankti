...
Color ParseColorString(const char * str, ColorStrError * err){
    ...
    double rawRed = NumberFromStr(clrValues[0], StrLength(clrValues[0]), &numParseOk);
    if (!numParseOk) {
    ...
    double rawGreen = NumberFromStr(clrValues[1], StrLength(clrValues[1]), &numParseOk);
    if (!numParseOk) {
    ...
    double rawBlue = NumberFromStr(clrValues[2], StrLength(clrValues[2]), &numParseOk);
    if (!numParseOk) {
    ...
    if (clrCodeCount == 4) {
        rawAlpha = NumberFromStr(clrValues[3], StrLength(clrValues[3]), &numParseOk);
        if (!numParseOk) {
