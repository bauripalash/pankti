...
static bool brlkbSetup(PanKbCtx *ctx) {
  ...
  kbts_ShapePushFontFromMemory(ctx->kbCtx, ctx->fontFileData, ctx->fontFileSize,
                               0);
  stbtt_InitFont(&ctx->sFontInfo, ctx->fontFileData,
                 stbtt_GetFontOffsetForIndex(ctx->fontFileData, 0));
  return true;
}

static void brlkbResetSetup(PanKbCtx *ctx) { 
...
bool PanKbCtxLoadFontFromMemory(PanKbCtx *ctx, unsigned char *data, size_t size) {
  if (ctx == NULL) {
    return false;
  }
  brlkbResetSetup(ctx);
...
bool PanKbLoadDefaultNotoFont(PanKbCtx * ctx){
    return PanKbCtxLoadFontFromMemory(ctx,NOTOSERIFBENGALI_TTF , NOTOSERIFBENGALI_TTF_LEN);
}

static void brlkbDrawGlyph(PanKbCtx *ctx, int index, int posX, int posY,
                           float scale, Color color) {
  int width, height, xoffset, yoffset;
  unsigned char *bitmap =
      stbtt_GetGlyphBitmap(&ctx->sFontInfo, scale, scale, (int)index, &width,
...
bool PanKbCtxDrawText(PanKbCtx *ctx, int xpos, int ypos, const char *text,
                   Color color) {
  kbts_ShapeBegin(ctx->kbCtx, KBTS_DIRECTION_LTR, KBTS_LANGUAGE_BANGLA);
  kbts_ShapeUtf8(ctx->kbCtx, text, strlen(text),
  ...
  while (kbts_ShapeRun(ctx->kbCtx, &Run)) {
    ...
    while (kbts_GlyphIteratorNext(&Run.Glyphs, &Glyph)) {
      ...
      brlkbDrawGlyph(ctx, glyphIndex, curX + gxOffset, curY + gyOffset, scale,
                     color);
      curX += gxAdvance;
      curY += gyAdvance;
    }
