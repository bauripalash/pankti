...
PObj *NewObject(Pgc * gc, PObjType type) {
	...
	gc->objects = o;
	#if defined DEBUG_GC
	printf(TERMC_BLUE "[DEBUG] [GC] %p New Object : %s\n" TERMC_RESET, o, ObjTypeToString(type));
	#endif
    return o;
}
...
void FreeObject(Pgc * gc, PObj *o) {
	...
	#if defined DEBUG_GC
	printf(TERMC_GREEN "[DEBUG] [GC] Freeing Object : %s : " TERMC_RESET, ObjTypeToString(o->type));
	PrintObject(o);
	printf("\n");
	#endif
